...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        for (int i = 3; i < argc; ++i) {
            ...
            // If an S is listed more than once, later inserts into the map will fail and be skipped silently.
    ...
    /* OpenMP to parallelize the first 3 levels of loop,
     * a total of (2Ss+1)*(2Ss+1)*(2Se+1) tasks that can be done in parallel.
     *  Not trying further because a lot of private variables besides loop indices. */
            ...
            for (twoMe = -twoSe; twoMe <= twoSe; twoMe +=2) {
                ...
                for (int twoSinta = std::max(abs(twoMa+twoMe), abs(twoSs-twoSe)); twoSinta <= twoSs+twoSe; twoSinta +=2) {
                    for (int twoSintb = std::max(abs(twoMb+twoMe), abs(twoSs-twoSe)); twoSintb <= twoSs+twoSe; twoSintb +=2) {
                         ...
                         * are possible. If yes, calculates the contribution by <Ss,Ma,Se,Me,Ss,Mb|T|Ss,Ma',Se,Me',Ss,Mb'>
                         * to the matrix element <Sa, M| T |Sb, M'> in the couple states basis. */
                        for (auto&& twoSa : twoS_values ) {
                            for (auto&& twoSb : twoS_values) {
                                if (twoSa >= abs(twoM) && twoSa >= abs(twoSinta-twoSs) && twoSa <= twoSinta+twoSs
                                    ...
                                    // Will lead to race condition if multiple threads try to update t_matrix.
                                    #pragma omp atomic
